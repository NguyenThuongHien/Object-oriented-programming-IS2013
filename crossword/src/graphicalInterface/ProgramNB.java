            ...
            if (browser.hasActual()) {
                ...
                for (int i = 0; i < browser.getActual().getBoardWidth(); i++) {
                    for (Integer j = 1; j <= browser.getActual()
                            .getBoardHeight(); j++) {
                        if (browser.getActual().getStrategyID() == Strategy.easyStrategyID) {
                            graphic.drawString(j.toString() + ". ", 10,
                                    30 + (j - 1) * 30);
                        } else {
                            Iterator<CwEntry> iter = browser.getActual().getROEntryIter();
                            int k = 1;
                            int l = 1;
                            while (iter.hasNext()) {
                ...
                int j = 0;
                for (String entry : browser.printAllEntries().split("\n")) {
                    graphic.drawString(entry, 10, browser.getActual()
                            .getBoardHeight() * 30 + 50 + j * 15);
                ...
                if (this.getSize().width != maxWidth
                    ...
                    this.setPreferredSize(new Dimension(maxWidth, j * 16
                            + browser.getActual().getBoardHeight() * 30 + 38));
                }
            }
        }
        ...
        public int print(Graphics graphic, PageFormat pf, int page)
            ...
            for (int i = 0; i < browser.getActual().getBoardWidth(); i++) {
                for (Integer j = 1; j <= browser.getActual()
                        .getBoardHeight(); j++) {
                    if (browser.getActual().getStrategyID() == Strategy.easyStrategyID) {
                        graphic.drawString(j.toString() + ". ", 10 + constX,
                                30 + (j - 1) * 30 + constY);
                    } else {
                        Iterator<CwEntry> iter = browser.getActual().getROEntryIter();
                        int k = 1;
                        ...
                        while (iter.hasNext()) {
                            CwEntry entry = iter.next();
                            if (entry.getDir() == CwEntry.Direction.VERT) {
                                graphic.drawString(k + ". ", entry.getX() * 30 + 45 + constX,
                                        20 + entry.getY() * 30 + constY);
                                k++;
                            } else {
                                graphic.drawString(l + ". ", entry.getX() * 30 + 15 + constX,
                                        50 + entry.getY() * 30 + constY);
                                l++;
                            }
                        }
                    }
            ...
            for (String entry : browser.printAllEntries().split("\n")) {
                ...
                graphic.drawChars(entry.toCharArray(), (temp - j) * 70, 70, 10 + constX, browser.getActual()
                        .getBoardHeight() * 30 + 50 + j * 15 + constY);
                j++;
                temp += 2;
                }
                graphic.drawChars(entry.toCharArray(), (temp - j) * 70, entry.length() - (temp - j)*70, 10 + constX, browser.getActual()
                        .getBoardHeight() * 30 + 50 + j * 15 + constY);
                j++;
                temp += 2;
            }
        ...
        public void paintSolved(Graphics graphic) {
            ...
            for (int i = 0; i < browser.getActual().getBoardWidth(); i++) {
                for (Integer j = 1; j <= browser.getActual()
                        .getBoardHeight(); j++) {
                    if (browser.getActual().checkBoardCell(i, j - 1)) {
                        graphic.drawString(browser.getActual().getBoardCopy().getCell(i, j - 1).getContent(), i * 30 + 46, (j - 1) * 30 + 50
                        );
                    }
                }
            }
